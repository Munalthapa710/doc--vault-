...
[Authorize]
[Route("api/settings")]
public class SettingsController(MongoContext context, IAuthService authService, ITokenService tokenService) : BaseApiController
{
    [HttpGet("profile")]
    ...
    {
        var update = Builders<Models.User>.Update.Set(x => x.FullName, request.FullName.Trim()).Set(x => x.UpdatedAt, DateTime.UtcNow);
